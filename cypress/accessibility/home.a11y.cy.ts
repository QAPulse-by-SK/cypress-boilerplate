/**
 * Accessibility Tests — cypress-axe + A11yHelper
 * QA Pulse by SK - www.skakarh.com
 * Tags: @a11y @regression @smoke (critical)
 */
...
describe("A11y — skakarh.com @a11y", () => {
  ...
  it("full WCAG 2.1 AA scan — no violations @a11y @regression @critical", () => {
  ...
  it("all images have alt text @a11y @regression", () => {
    A11yHelper.assertImagesHaveAltText();
  });

  it("page has exactly one H1 @a11y @regression", () => {
    A11yHelper.assertSingleH1();
  });

  it("ARIA landmarks present @a11y @regression", () => {
    A11yHelper.assertAriaLandmarks();
  });

  it("no colour contrast violations @a11y @regression", () => {
    A11yHelper.assertNoViolations({ tags: ["cat.color"] });
  });
});
...
describe("A11y — Login Page @a11y", () => {
  ...
  it("no critical violations @a11y @smoke @critical", () => {
    cy.logStep("Critical a11y scan on login page");
    A11yHelper.assertNoCriticalViolations({ tags: ["wcag2a", "wcag2aa"] });
  });

  it("login form inputs have labels @a11y @regression", () => {
    A11yHelper.assertFormLabels();
  });

  it("login form component scoped scan @a11y @regression", () => {
    A11yHelper.assertComponentAccessible("#login");
  });
  ...
  it("can submit login form with keyboard only @a11y @regression", () => {
    cy.logStep("Keyboard-only login");
...
describe("A11y — Home Page @a11y", () => {
  ...
  it("no critical violations @a11y @smoke", () => {
    A11yHelper.assertNoCriticalViolations();
  });

  it("home page has a single H1 @a11y @regression", () => {
    A11yHelper.assertSingleH1();
  });

  it("cy.checkA11yFull command works @a11y @regression", () => {
    cy.checkA11yFull({ tags: ["wcag2a", "wcag2aa"] });
  });

  it("cy.checkA11yCritical command works @a11y @smoke", () => {
